...
> **Prefix:** gme
> **Status:** Draft (Phase B round 1 — derived from signed BRD v0.3 +
...
| max_pain                | Strike minimising aggregate ITM dollar OI for the nearest unexpired weekly expiry, with strikes deduplicated per side before summation.                                                                                                                                     | derived     | exact              | `argmin_K Σ_calls oi · greatest(0, K_under − K) + Σ_puts oi · greatest(0, K − K_under)` over distinct strikes per side per expiry (see T-13 `gme_dws_perf_max_pain` for the SQL).                                                                                                                                                              | `gme_ads_market_dashboard`               |
...
| iv_rank                 | Percentile of current iv30 within trailing 252-trading-day distribution of iv30 (0–100).                                                                                                                                                                                     | derived     | phase-gated        | `100 · percent_rank(iv30) over (order by iv30 rows 251 preceding) at trading_date = today` against `gme_dws_perf_implied_vol`. Carries `iv_rank_label` ∈ {provisional, final} and `iv_rank_lookback_days` (rolling count of non-null iv30). See T-13 `gme_dws_perf_implied_vol` and `gme_ads_market_dashboard`.                                  | `gme_ads_market_dashboard`               |
| dealer_net_gamma        | Σ over **front-month expiry only** of `γ · OI · 100 · sign_dealer(type)`, shares per 1% spot move.                                                                                                                                                                            | derived     | unsupported        | `SUM(gamma_bs · open_interest · 100 · sign_dealer)` against `gme_dwd_options_chain_greeks WHERE expiry_date = front_expiry_date`. See T-13 `gme_dws_perf_dealer_gamma_front_month`.                                                                                                                                                              | `gme_ads_market_dashboard`               |
...
| ADS   | `gme_ads_market_dashboard`                 | table            | `(trading_date)` — one row per latest available trading_date   | Wide single-row presentation table that backs the Streamlit dashboard, with one column per §T-3 metric, plus  freshness fields and per-metric `link_status_active`. |
...
| front_expiry_flag       | BOOLEAN   | True iff `expiry_date = min(expiry_date) over (partition by trading_date)`.                | TRUE               | `expiry_date = min(expiry_date) over (partition by trading_date)`                                                                                                                          | derived                                      |
...
| max_pain_strike    | DOUBLE    | Strike minimising aggregate ITM dollar OI for this (date, expiry). Per-side dedup first. | 24.0          | Two-CTE form: (1) `WITH dedup AS (SELECT trading_date, expiry_date, strike, option_type, sum(open_interest) AS oi FROM gme_dwd_options_chain GROUP BY 1,2,3,4)`; (2) for each strike `K` in the distinct strike universe at `(trading_date, expiry_date)`, compute `pain(K) = SUM_calls(oi * GREATEST(0, K_under - K)) + SUM_puts(oi * GREATEST(0, K - K_under))` where `K_under` iterates over the same distinct strike universe; (3) `argmin_K pain(K)`. CRITICAL: per-side strike dedup BEFORE the cross join with the candidate-strike universe, otherwise the cardinality bug from predecessor `bae4af2` returns. Implemented as a singular dbt test fixture too. | derived                              |
...
|--------------------------|-----------|-----------------------------------------------------------------------------------------------------------------------------|---------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------|
| trading_date             | DATE      | Session date.                                                                                                                | 2026-06-02    | pass-through from `gme_dwd_options_chain.trading_date`                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | dwd                                        |
| iv30                     | DOUBLE    | Linear-in-total-variance interpolation of OI-weighted ATM IV to a constant 30-cal-day tenor, annualised decimal.              | 0.85          | Step 1: per expiry, `iv_atm = SUM(oi · sigma) / SUM(oi)` over the ATM band `|strike/spot − 1| ≤ 0.05` across both call+put rows. Step 2: identify the two bracketing expiries `(T_near, T_far)` with `T_near ≤ 30/365 < T_far`. Step 3: `v_T = pow(iv_atm_T, 2) · T_T`; Step 4: `v_30 = v_near + (v_far − v_near) · ((30/365 − T_near) / (T_far − T_near))`. Step 5: `iv30 = sqrt(v_30 / (30/365))`. If only one expiry exists with `T ≥ 30/365`, return its `iv_atm` directly. If no expiry, NULL. (BRD §B-3 iv30 row.)               | derived                                    |
...
| iv_rank                  | DOUBLE    | Percentile (0–100) of current `iv30` within the trailing 252 non-null `iv30` values. NULL while `iv_rank_lookback_days < 252`. | 67.2          | `CASE WHEN iv_rank_lookback_days >= 252 THEN 100 * percent_rank() over (order by iv30 rows 251 preceding) ELSE NULL END`                                                                                                                                                                                                                                                                                                                                                                                                       | derived                                    |
...
| max_pain_strike_front             | DOUBLE    | Front-expiry `max_pain_strike` for the dashboard tile.                                                                | 24.0                  | `(SELECT max_pain_strike FROM gme_dws_perf_max_pain WHERE trading_date = ads.trading_date AND expiry_date = (SELECT MIN(expiry_date) FROM gme_dwd_options_chain WHERE trading_date = ads.trading_date AND expiry_date > ads.trading_date))` | dws                                          |
...
| most_recent_session_close_ts_utc  | TIMESTAMP | 21:00 UTC on the latest `dim_date.calendar_date` with `is_trading_day=TRUE` that is `<= now()`.                          | 2026-06-02 21:00:00   | `(SELECT MAX(calendar_date) FROM dim_date WHERE is_trading_day = TRUE AND calendar_date <= current_date) + INTERVAL '21 hours'`                                                    | dim                                          |
| pull_lag_hours                    | DOUBLE    | Signed hours: `(last_pull_ts_utc - most_recent_session_close_ts_utc) / 1h`. Drives T2.1/T2.3 banner.                     | 0.09                  | `extract(epoch from (last_pull_ts_utc - most_recent_session_close_ts_utc)) / 3600.0`                                                                                                | derived                                      |
| is_stale                          | BOOLEAN   | TRUE iff T2.1 inequality is false (drives STALE banner per BRD L-7 round-2-itemA wording).                              | FALSE                 | `NOT (pull_lag_hours >= 0 AND pull_lag_hours <= 26)`                                                                                                                                | derived                                      |
| date_sk                           | INTEGER   | FK → `dim_date.date_sk`.                                                                                                | 20260602              | `cast(strftime(trading_date, '%Y%m%d') as integer)`                                                                                                                                  | derived                                      |
...
| `gme_ads_market_dashboard`                        | table            | --              | --                           |
...
| `gme_ads_market_dashboard`                        | 1 row                                         | 1 row (latest only — full refresh)          |
...
|------------------------------------------------|------------|-------------------------------------------------------------------------|-------------------------------|---------------------------------------------------------------------------------------------------------------------------|
| Header banner                                  | Text       | `trading_date`, `last_pull_ts_utc`, `pull_lag_hours`, STALE flag         | (none)                        | Connection banner: "Connected to MotherDuck" (live) / "local DuckDB fixture mode" (fallback). STALE banner if `is_stale`.   |
...
| TL-4 | Carry-forward of BRD L-7: freshness must be a single inequality. T2.1/T2.2/T2.3 in TEST PLAN evaluate the same predicate on `pull_lag_hours`. | Producer/validator drift on "pull age" semantics killed round-1 dashboard behavior. | `gme_ads_market_dashboard.is_stale = NOT (pull_lag_hours >= 0 AND pull_lag_hours <= 26)`; TC-12 enforces.                                                                                                                                                                                                                                                                     |
...
|---------------|----------------------------|------------|------------------------------------------------------------------------------------------|
| Stakeholder   | ________________            | __________ | (unsigned — Phase B.5 review pending)                                                    |
| Data Engineer | ________________            | __________ | (unsigned — Phase B.5 review pending)                                                    |
