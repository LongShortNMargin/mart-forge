...
### Phase D /mart-dqc audit outcome (8-control)

> Single live audit run: 35 / 36 controls **pass**, 1 marked
> **not_applicable** (no waivers). The NA row is recorded here per the
> SPEC §6.3 exhaustion protocol so it is visible at the business-
> requirements layer, not buried in `dqc_scorecard.json`.

| control_id                      | control_class            | table                       | status         | Rationale + Resource-Exhaustion Evidence                                                                                                                                                                                                                                                                                                                                       |
|---------------------------------|--------------------------|-----------------------------|----------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| business_recon_t1_6b_rate_floor | Business Reconciliation  | `gme_dws_perf_dealer_gamma` | not_applicable | T1.6b parametric rate-sensitivity (TDD §T-18 row `business_recon_t1_6b_floor` / §T-19 TC-14) sweeps the producer formula at `r ∈ {0.03, 0.045, 0.06}` with a `$1e6` denominator floor. The dbt warehouse output is pinned to `r = 0.045` via `vars.risk_free_rate`, so a single dbt test against the materialised row cannot express the multi-rate sweep — the parametric guarantee is asserted by the CI-side Python test `tests/test_net_gex_rate_sensitivity.py` (queued as Phase D follow-up alongside the Fred 3M T-bill ingest from OQ-2). Exhaustion: dbt's `accepted_range`, `relationships`, and singular-test forms each express **one** assertion against the **one** materialised row — none can iterate over hypothetical rate inputs without a parametric harness, and writing a hand-rolled DuckDB CROSS JOIN against a synthetic rate grid would couple the test to a producer-shape that does not exist in the warehouse contract. Tracked under TDD T-21 / TL-2; not a quality gap, a tooling-boundary call. |
