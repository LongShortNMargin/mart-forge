...
> **Prefix:** gme
> **Status:** Signed (v0.3 — Phase B.5 round-2 approved at comment
> `1bd13822`, conditional on item A: iv_rank percentile window. Item A
> patched in this commit per orchestrator directive `fcd99bc4` — T-13
> percentile LEFT JOIN now uses the dim_date OFFSET-251 trading-day
> form, pinning the rolling window to exactly 252 prior trading days
> per BRD §B-3. TC-17 fixture extended 252 → 400 days to expose any
> future window drift. `tdd_signed: true`; downstream phases unblocked.)
...
| 0.3     | 2026-06-02 | mart-forge example author  | T-13, T-19                                       | Phase B round-2 item-A patch + TDD signing per orchestrator directive `fcd99bc4` + reviewer `1bd13822` (approve, conditional on item A). T-13 `iv_rank` percentile LEFT JOIN replaced the calendar-day `INTERVAL '500 days'` window with the trading-day `dim_date` OFFSET-251 form so the join slice pins to exactly 252 prior trading days per BRD §B-3's strict contract (round-2 draft over-sampled to ~344 trading days in steady state). TC-17 fixture extended from 252 → 400 days so the test exposes any future drift between the implementation window and the BRD contract. `tdd_signed: true`; `mart.yml.phase = B_signed`. |
...
| iv_rank                  | DOUBLE    | Percentile (0–100) of current `iv30` within the trailing 252 non-null `iv30` values, ranked chronologically. NULL while `iv_rank_lookback_days < 252`. | 67.2          | Chronological rolling percentile (closes reviewer Phase B.5 finding 2). Implemented via a self-join against the trailing 252-trading-day slice: `WITH base AS (SELECT trading_date, iv30 FROM {{ ref('gme_dws_perf_implied_vol') }} WHERE iv30 IS NOT NULL), counted AS (SELECT b1.trading_date, b1.iv30, count(*) FILTER (WHERE b2.iv30 <= b1.iv30 AND b2.trading_date < b1.trading_date) AS rank_count, count(*) FILTER (WHERE b2.trading_date < b1.trading_date) AS denom FROM base b1 LEFT JOIN base b2 ON b2.trading_date >= (SELECT calendar_date FROM {{ ref('dim_date') }} WHERE is_trading_day = TRUE AND calendar_date < b1.trading_date ORDER BY calendar_date DESC OFFSET 251 LIMIT 1) AND b2.trading_date < b1.trading_date AND b2.iv30 IS NOT NULL GROUP BY 1,2) SELECT trading_date, CASE WHEN iv_rank_lookback_days >= 252 THEN 100.0 * rank_count / NULLIF(denom, 0) ELSE NULL END AS iv_rank FROM counted JOIN ... USING (trading_date)`. Window ORDERs BY `trading_date`, percentile counts *prior* iv30 values ≤ current. NOT `percent_rank() OVER (ORDER BY iv30)` — that ordering is by magnitude, not time, and produces a degenerate measure of the current row's position within its own iv30-sorted frame. The `b2.trading_date >= (SELECT … FROM dim_date … ORDER BY calendar_date DESC OFFSET 251 LIMIT 1)` form (closes Phase B.5 round-2 item A) pins the join slice to **exactly 252 prior trading days** per the BRD §B-3 contract, replacing the round-2 draft's `INTERVAL '500 days'` calendar window which over-sampled to ~344 trading days in steady state. `dim_date` provides the trading-day calendar (`is_trading_day = TRUE`), so the OFFSET-251 lookup returns the 252nd-most-recent trading_date strictly before `b1.trading_date`, and the inclusive lower bound + exclusive upper bound make the slice closed on the left and open on the right. | derived                                    |
...
| column_name                       | data_type | definition                                                                                                            | example_value         | calculation                                                                                                                                                                       | data_source                                  |
|-----------------------------------|-----------|-----------------------------------------------------------------------------------------------------------------------|-----------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------|
| trading_date                      | DATE      | Most recent trading_date present in both `gme_dwd_price_eod` and `gme_dwd_options_chain`, restricted to rows whose `pull_ts_utc <= now()` (excludes future-dated fixture rows by construction; closes Phase C.5 round-2 finding 1). | 2026-06-02            | `SELECT MAX(p.trading_date) FROM gme_dwd_price_eod p INNER JOIN (SELECT DISTINCT trading_date FROM gme_dwd_options_chain WHERE pull_ts_utc <= now()) c ON p.trading_date = c.trading_date WHERE p.pull_ts_utc <= now()` | dwd                                          |
...
| TC-17   | DWS   | `gme_dws_perf_implied_vol`                      | singular            | `iv_rank_fixture_synthetic_400d.sql` — synthetic **400-trading-day** iv30 series with a known monotonic distribution; assert `iv_rank` of the last row matches the analytic percentile of the trailing-252-day slice within ±0.1. Fixture length deliberately exceeds 252 (closes Phase B.5 round-2 item A) so the test exposes any drift between the implementation's window and the BRD's strict 252-trading-day contract — a 252-only fixture sits exactly at the boundary where a 500-calendar-day window happens to coincide with 252 trading days and the drift is invisible. Would FAIL under `percent_rank() OVER (ORDER BY iv30)` (closes Phase B.5 finding 2) AND under the round-2-draft 500-calendar-day window when run against this 400-day fixture (closes round-2 item A). | 0 failures      |
...
|---------------|----------------------------|------------|------------------------------------------------------------------------------------------|
| Stakeholder   | mart-forge example author  | 2026-06-02 | signed v0.3 — Phase B.5 round-2 approved at comment `1bd13822`; item A patched per `fcd99bc4` |
| Data Engineer | mart-forge example author  | 2026-06-02 | signed v0.3 — Phase B.5 round-2 approved at comment `1bd13822`; item A patched per `fcd99bc4` |
