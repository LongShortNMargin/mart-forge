...
<!-- Reference the BRD for full context. Summarize the key business
process and analytical purpose here. -->

_TODO: Brief recap of the business process, stakeholders, and
analytical goals from the BRD._
...
_TODO: Define the most atomic level of data stored in the fact table
(e.g., one row per transaction per day)._

### Step 3: Identify the Dimensions

_TODO: List dimension tables and their role (dim_date for time-series
slicing, dim_product for product attributes)._

### Step 4: Identify the Facts

_TODO: List the measurable, numeric facts captured at the declared
grain._
...
- Derived columns: actual SQL or formula. No "derived", "computed",
  "see model" placeholders.
- Hybrid columns: both the native component reference AND the
  derivation formula AND the reconciliation tolerance.
...
| Property              | Value                                          |
|-----------------------|------------------------------------------------|
| source                | _TODO: provider + endpoint/method_             |
...
| backfill              | _TODO: initial load command and window_        |
...
| column_name   | data_type   | definition                        | example_value       | calculation | data_source   |
|---------------|-------------|-----------------------------------|---------------------|-------------|---------------|
| _TODO_        |             |                                   |                     | pass-through from <provider>.<field> |               |
| provider      | VARCHAR     | Name of the data provider/source  | 'api_provider_name' | pass-through from system | system |
| pull_ts_utc   | TIMESTAMP   | UTC timestamp of data extraction  | 2026-01-15 08:30:00 | pass-through from system | system |
| run_id        | VARCHAR     | Unique identifier for the ETL run | 'run_20260115_0830' | pass-through from system | system |
...
| column_name        | data_type | definition                   | example_value | calculation                              | data_source |
|--------------------|-----------|------------------------------|---------------|------------------------------------------|-------------|
| <entity>_sk        | INTEGER   | Surrogate key                | 1             | row_number() over (order by <natural>)  | generated   |
| _TODO_             |           |                              |               |                                          |             |
...
| column_name   | data_type   | definition                        | example_value       | calculation                  | data_source   |
|---------------|-------------|-----------------------------------|---------------------|------------------------------|---------------|
| _TODO_        |             |                                   |                     | SUM(<col>) / AVG(<col>) / PERCENTILE_CONT(0.5) |  |
...
_TODO: Estimate row counts and storage per table for the initial load
and steady-state daily increments._
...
|-----------------|----------------|------------------------|--------------------------|---------------------|
| _TODO_          | Line / Bar     |                        | date_range, entity       | per metric badge    |
...
|----------------------|-------|------------------------|---------------------------------|-----------------|
| pk_integrity         | ALL   | <each table>           | PK unique + not null            | 0 violations    |
...
| accepted_range       | DWS   | {prefix}_dws__         | numeric within plausible bounds | per column      |
| duplicate_detection  | DWD   | {prefix}_dwd__         | no duplicate business keys      | 0 duplicates    |
| business_recon       | ADS   | {prefix}_ads__         | matches external source         | tolerance %     |
...
> Carry forward any items from BRD §B-4 and add any new technical
> limitations discovered during design.
